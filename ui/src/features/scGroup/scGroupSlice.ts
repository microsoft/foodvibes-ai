...
import { getScGroupRows, putScGroup } from "./scGroupAPI";

const name: string = KLedgerTypeScGroup;
const initialState: FeatureSliceState<ScGroupType> =
    GetFeatureInitialState<ScGroupType>(name);

export const scGroupSlice = createAppSlice({
    name,
    initialState,
    reducers: create => ({
        ...
        actionSelectScGroup: create.asyncThunk(
            async ({ queryParams, accessToken, instance, }: { queryParams: QueryParamsType; accessToken: string | null; instance: IPublicClientApplication; }, { dispatch }) => {
                ...
                const response = await getScGroupRows(queryParams, acccessTokenToUse);
        ...
        actionUpsertScGroup: create.asyncThunk(
            async ({
                queryParams,
                rowToUpsert,
                accessToken,
                ...
                const response = await putScGroup(queryParams, rowToUpsert, acccessTokenToUse);
    ...
    selectors: {
        ...
        selectScGroupIsLoading: state => state.loading,
        ...
        selectLastIdScGroup: state => state.lastId,
        selectScGroupResponse: state => state.queryResponse,
    },
});
