...
def _render_column(column, show_name):
    ...
    is_unique = any(
        isinstance(c, UniqueConstraint) and set(c.columns) == set([column])
        for c in column.table.constraints
    )
    is_unique = is_unique or any(
        i.unique and set(i.columns) == set([column]) for i in column.table.indexes
    )
    ...
    return _flask_prepend + "Column({0})".format(
        ", ".join(
            ...
            + (["info={!r}".format(comment)] if comment else [])  # type: ignore
        )  # type: ignore
    )  # type: ignore
...
def _render_constraint(constraint):
    ...
    if isinstance(constraint, ForeignKey):
        remote_column = "{0}.{1}".format(
            constraint.column.table.fullname, constraint.column.name
        )
        return _flask_prepend + "ForeignKey({0})".format(
            render_fk_options(remote_column)
        )
    elif isinstance(constraint, ForeignKeyConstraint):
        local_columns = _get_column_names(constraint)
        remote_columns = [
            "{0}.{1}".format(fk.column.table.fullname, fk.column.name)
            for fk in constraint.elements
        ]
        return _flask_prepend + "ForeignKeyConstraint({0})".format(
            render_fk_options(local_columns, remote_columns)
        )
    elif isinstance(constraint, CheckConstraint):
        return _flask_prepend + "CheckConstraint({0!r})".format(
            _get_compiled_expression(constraint.sqltext)
        )
...
class ImportCollector(OrderedDict):
    def add_import(self, obj):
        ...
        pkgname = (
            "sqlalchemy" if type_.__name__ in sqlalchemy.__all__ else type_.__module__
        )  # @UndefinedVariable
...
class ModelClass(Model):
    ...
    def __init__(
        self, table, association_tables, inflect_engine, detect_joined, collector
    ):
            ...
            if _dataclass:
                if column.type.python_type.__module__ != "builtins":
                    collector.add_literal_import(
                        column.type.python_type.__module__,
        ...
        for association_table in association_tables:
            ...
            relationship_ = ManyToManyRelationship(
                self.name, target_cls, association_table, inflect_engine
            )
            self._add_attribute(relationship_.preferred_name, relationship_)
    ...
    def _tablename_to_classname(tablename, inflect_engine):
        camel_case_name = "".join(
            part[:1].upper() + part[1:] for part in re.split(r"_|-", tablename)
        )
    ...
    def render(self):
        ...
        kwargs_items = ", ".join(
            "{0!r}: {1!r}".format(key, table_kwargs[key]) for key in table_kwargs
        )
        ...
        for attr, column in self.attributes.items():
            if isinstance(column, Column):
                ...
                if _dataclass:
                    text += (
                        "    " + attr + " : " + column.type.python_type.__name__ + "\n"
                    )
...
class Relationship(object):
    ...
    def make_backref(self, relationships, classes):
        ...
        while (self.target_cls, backref) in [
            (x.target_cls, x.backref_name) for x in relationships
        ]:
        ...
        for rel in [x for x in relationships if "backref" in x.kwargs]:
            if self.target_cls in classes and rel.target_cls in classes:
                if _is_model_descendant(
                    classes[self.target_cls], classes[rel.target_cls]
                ):
                    self.backref_name = self.target_cls.lower() + "_" + backref
                    self.kwargs["backref"] = repr(self.backref_name)
                if _is_model_descendant(
                    classes[rel.target_cls], classes[self.target_cls]
                ):
...
class ManyToOneRelationship(Relationship):
    def __init__(self, source_cls, target_cls, constraint, inflect_engine):
        ...
        if source_cls == target_cls:
            self.preferred_name = (
                "parent" if not colname.endswith("_id") else colname[:-3]
            )
        ...
        # common_fk_constraints = _get_common_fk_constraints(
        # constraint.table, constraint.elements[0].column.table)
        # if len(common_fk_constraints) > 1:
        # self.kwargs['primaryjoin'] = "'{0}.{1} == {2}.{3}'".format(source_cls,
        # constraint.columns[0], target_cls, constraint.elements[0].column.name)
        if len(constraint.elements) > 1:
            ...
            self.kwargs["primaryjoin"] = "'and_({0})'".format(
                ", ".join(
                    [
                        "{0}.{1} == {2}.{3}".format(
                            source_cls, k.parent.name, target_cls, k.column.name
                        )
...
class ManyToManyRelationship(Relationship):
    def __init__(self, source_cls, target_cls, assocation_table, inflect_engine):
        ...
        prefix = (
            assocation_table.schema + "." if assocation_table.schema is not None else ""
        )
        self.kwargs["secondary"] = repr(prefix + assocation_table.name)
        constraints = [
            c
            for c in assocation_table.constraints
            if isinstance(c, ForeignKeyConstraint)
        ]
        ...
        self.preferred_name = (
            tablename if NoReturn != colname.endswith("_id") else colname[:-3] + "s"
        )  # type: ignore
        self.backref_name = inflect_engine.plural_noun(self.backref_name)
        ...
        if source_cls == target_cls:
            self.preferred_name = (
                "parents" if NoReturn != colname.endswith("_id") else colname[:-3] + "s"
            )  # type: ignore
            ...
            pri_joins = [
                "{0}.{1} == {2}.c.{3}".format(
                    source_cls, elem.column.name, assocation_table.name, col
                )
                for col, elem in pri_pairs
            ]
            sec_joins = [
                "{0}.{1} == {2}.c.{3}".format(
                    target_cls, elem.column.name, assocation_table.name, col
                )
...
class CodeGenerator(object):
    ...
    def __init__(
        ...
        for table in metadata.tables.values():
            ...
            # them except for special columns like id, inserted, and updated
            fk_constraints = [
                constr
            ...
            if len(fk_constraints) == 2 and all(
                ...
                if col.name not in _ignore_columns
            ):
                association_tables.add(table.name)
                tablename = (
                    sorted(fk_constraints, key=_get_constraint_sort_key)[0]
                    .elements[0]
                    .column.table.name
                )
            ...
            if noconstraints:
                ...
                for constraint in table.constraints.copy():
                    if isinstance(constraint, CheckConstraint):
                        ...
                        if match:
                            colname = _re_column_name.match(match.group(1)).group(3)  # type: ignore
                            table.constraints.remove(constraint)
                            table.c[colname].type = Boolean()
                            continue

                        # Turn any string-type column with a CheckConstraint like "column IN (...)"
                        # into an Enum
    ...
    def render(self, outfile=sys.stdout):
        ...
        logger.info(self.collector.render() + "\n\n")

        if self.flask:
            logger.info("db = SQLAlchemy()")
        else:
            if any(isinstance(model, ModelClass) for model in self.models):
                logger.info("Base = declarative_base()\nmetadata = Base.metadata")
            else:
        ...
        for model in self.models:
            logger.info("\n\n")
            logger.info(model.render().rstrip("\n"))

        if self.footer:
            logger.info(self.footer)
